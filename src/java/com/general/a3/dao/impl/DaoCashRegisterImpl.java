...
public class DaoCashRegisterImpl implements Serializable{
    ...
    public Sic4cuaddiario getSummaryOrders(Session session, Sic4cuaddiario obj) throws Exception {
        ...
        String sql =                     
            "   SELECT SUM(CASE WHEN SCLASE.COD_SCLASEEVEN = 'VI_SICSCLASEEVENVENTA' AND GEN.COD_VALORGENERAL NOT IN ('VI_SICTRANSFE','VI_SICDEPOSITO') THEN NVL(S1.NUM_MTOEFECTIVO,0) - NVL(S1.NUM_MTOVUELTO,0) " +
            "                   WHEN SCLASE.COD_SCLASEEVEN = 'VI_SICSCLASEEVENNOTACREDITO' THEN -1 * NVL(S1.NUM_MTOEFECTIVO,0) ELSE 0 END) AS NUM_MTOEFECTIVO  " +
            ...
            "        ,SUM(CASE WHEN SCLASE.ID_CLASEEVEN = 46126 AND S1.ID_MODAPAGO = 46103 THEN NVL(S1.NUM_MTOEFECTIVO,0) - NVL(S1.NUM_MTOVUELTO,0) " +
            "                  WHEN SCLASE.COD_SCLASEEVEN = 'VI_SICSCLASEEVENCOMPRA' THEN NVL(S1.NUM_MTOEFECTIVO,0) ELSE 0 END) AS NUM_MTOGASTOEFECTIVO " +
            "        ,NVL(SUM(CASE WHEN SCLASE.COD_SCLASEEVEN = 'VI_SICSCLASEEVENVENTA' AND S1.ID_MODAPAGO IN (46101,46102) THEN NVL(S1.NUM_MTOEFECTIVO,0)  ELSE 0 END),0) AS NUM_MTOTOTALTRANDEPO  " +
            "  FROM SIC1DOCU S1 " +
